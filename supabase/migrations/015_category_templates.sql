-- Migration 015: Link categories to their own templates + add AI hints to fields
--
-- Changes:
--   spec_categories        — adds template_id (FK to spec_templates)
--   spec_template_fields   — adds ai_hint text column
--
-- New function: seed_category_templates(p_studio_id uuid)
--   Creates one template per top-level category with pre-populated fields,
--   then links the category rows to their templates via template_id.
--   Call after seed_default_spec_categories() for new studios, or run once
--   for existing studios that already have the 10 default categories.

ALTER TABLE spec_categories
  ADD COLUMN IF NOT EXISTS template_id uuid REFERENCES spec_templates(id) ON DELETE SET NULL;

ALTER TABLE spec_template_fields
  ADD COLUMN IF NOT EXISTS ai_hint text;

-- ── seed_category_templates ───────────────────────────────────────────────────

CREATE OR REPLACE FUNCTION seed_category_templates(p_studio_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  -- ── Flooring ──────────────────────────────────────────────────────────────
  INSERT INTO spec_templates (studio_id, name, is_active)
    VALUES (p_studio_id, 'Flooring', true) RETURNING id INTO tmpl_flooring;

  INSERT INTO spec_template_fields (template_id, name, field_type, is_required, order_index, ai_hint) VALUES
  ...
  UPDATE spec_template_fields SET options = '["Class 1","Class 2","Class 3","Cfl-s1","Bfl-s1","Afl"]'::jsonb
    WHERE template_id = tmpl_flooring AND name = 'Fire Rating';
  UPDATE spec_template_fields SET options = '["R9","R10","R11","R12","R13","PTV 36+","PTV 45+"]'::jsonb
    WHERE template_id = tmpl_flooring AND name = 'Slip Rating';

  UPDATE spec_categories SET template_id = tmpl_flooring
  ...
  UPDATE spec_template_fields SET options = '["Class A","Class B","Class 1","Class 0","BS EN 13501 B-s1","BS EN 13501 C-s2"]'::jsonb
    WHERE template_id = tmpl_walls AND name = 'Fire Rating';

  UPDATE spec_categories SET template_id = tmpl_walls
  ...
  UPDATE spec_template_fields SET options = '["LED","Fluorescent","Halogen","Incandescent","Metal Halide","OLED","Fibre Optic"]'::jsonb
    WHERE template_id = tmpl_lighting AND name = 'Light Source';
  UPDATE spec_template_fields SET options = '["2700K Warm White","3000K Warm White","3500K Neutral White","4000K Cool White","5000K Daylight","Tuneable White"]'::jsonb
    WHERE template_id = tmpl_lighting AND name = 'Colour Temperature';
  UPDATE spec_template_fields SET options = '["IP20","IP44","IP54","IP65","IP67","IP68"]'::jsonb
    WHERE template_id = tmpl_lighting AND name = 'IP Rating';

  UPDATE spec_categories SET template_id = tmpl_lighting
  ...
  UPDATE spec_categories SET template_id = tmpl_furniture
  ...
  UPDATE spec_template_fields SET options = '["BS 5852 Crib 5","BS 5852 Crib 7","IMO A.652(16)","California TB117","EN 1021-1","EN 1021-2","Inherently FR"]'::jsonb
    WHERE template_id = tmpl_fabric AND name = 'Fire Rating';
  UPDATE spec_template_fields SET options = '["W — Water-based","S — Solvent-based","WS — Water or Solvent","X — Vacuum Only","Dry Clean Only"]'::jsonb
    WHERE template_id = tmpl_fabric AND name = 'Cleaning Code';
  UPDATE spec_template_fields SET options = '["Upholstery","Drapery","Dual Purpose","Bedding","Outdoor","Wall Covering"]'::jsonb
    WHERE template_id = tmpl_fabric AND name = 'Application';

  UPDATE spec_categories SET template_id = tmpl_fabric
  ...
  UPDATE spec_template_fields SET options = '["Polished","Honed","Brushed","Tumbled","Sandblasted","Flamed","Bush Hammered","Matt Glazed","Gloss Glazed","Lappato"]'::jsonb
    WHERE template_id = tmpl_stone AND name = 'Finish';
  UPDATE spec_template_fields SET options = '["R9","R10","R11","R12","R13","PTV 36+","PTV 45+","A","B","C"]'::jsonb
    WHERE template_id = tmpl_stone AND name = 'Slip Rating';
  UPDATE spec_template_fields SET options = '["BIa — ≤0.5% (Porcelain)","BIb — 0.5–3%","BIIa — 3–6%","BIIb — 6–10%","BIII — >10%"]'::jsonb
    WHERE template_id = tmpl_stone AND name = 'Water Absorption';
  UPDATE spec_template_fields SET options = '["Internal Floor","Internal Wall","External Floor","External Wall","Wet Area","Pool Surround","Facade"]'::jsonb
    WHERE template_id = tmpl_stone AND name = 'Application';

  UPDATE spec_categories SET template_id = tmpl_stone
  ...
  UPDATE spec_categories SET template_id = tmpl_joinery
  ...
  UPDATE spec_template_fields SET options = '["Matt","Flat Matt","Eggshell","Satin","Semi-Gloss","Gloss","Soft Sheen","Dead Flat","Limewash"]'::jsonb
    WHERE template_id = tmpl_paint AND name = 'Finish';
  UPDATE spec_template_fields SET options = '["Zero VOC","Low VOC (<10 g/L)","Medium VOC (10–100 g/L)","High VOC (>100 g/L)","Exempt Compound"]'::jsonb
    WHERE template_id = tmpl_paint AND name = 'VOC Level';
  UPDATE spec_template_fields SET options = '["Brush","Roller","Spray","Brush & Roller","Airless Spray","HVLP Spray"]'::jsonb
    WHERE template_id = tmpl_paint AND name = 'Application Method';

  UPDATE spec_categories SET template_id = tmpl_paint
  ...
  UPDATE spec_template_fields SET options = '["Basin","WC","Bath","Shower Tray","Bidet","Urinal","Sink","Butler Sink"]'::jsonb
    WHERE template_id = tmpl_sanitary AND name = 'Type';
  UPDATE spec_template_fields SET options = '["Vitreous China","Fireclay","Acrylic","Cast Iron","Solid Surface","Stainless Steel","Resin","Stone Resin"]'::jsonb
    WHERE template_id = tmpl_sanitary AND name = 'Material';
  UPDATE spec_template_fields SET options = '["Floor Standing","Wall Hung","Semi-Recessed","Countertop","Under-Counter","Inset","Freestanding"]'::jsonb
    WHERE template_id = tmpl_sanitary AND name = 'Fixing Type';

  UPDATE spec_categories SET template_id = tmpl_sanitary
  ...
  UPDATE spec_template_fields SET options = '["Lever Handle","Pull Handle","Knob","Flush Pull","Cylinder","Lock","Hinge","Closer","Overhead Stop","Floor Spring","Letter Box","Numerals"]'::jsonb
    WHERE template_id = tmpl_hardware AND name = 'Type';
  UPDATE spec_template_fields SET options = '["Polished Brass","Satin Brass","Aged Brass","Brushed Nickel","Polished Nickel","Satin Chrome","Polished Chrome","Matt Black","Antique Bronze","PVD Gold","Satin Stainless"]'::jsonb
    WHERE template_id = tmpl_hardware AND name = 'Finish';
  UPDATE spec_template_fields SET options = '["Round Rose","Square Rose","Oval Rose","Bathroom Turn & Release","Keyhole Escutcheon","None"]'::jsonb
    WHERE template_id = tmpl_hardware AND name = 'Rose/Escutcheon';

  UPDATE spec_categories SET template_id = tmpl_hardware
